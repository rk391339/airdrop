...
const fetchAddresses = async (provider, blockStartNumber, path) => {
    ...
    for(;i<pairLength; i+=1) {
        ...
        for(var j = blockStartNumber; j < currentBlockNumber; j+=5000) {
            console.log(pairAddress, "currentBlockNumber", j)
            let mintArray = await slpPair.queryFilter(slpPair.filters.Transfer(), j, j+5000)
            let swapArray = await slpPair.queryFilter(slpPair.filters.Swap(), j, j+5000)
